...
public class JuheSmsClient {
    ...
    public SendResult send(String mobile, String code) {
        ...
        try {
            ...
            // 用 UriComponentsBuilder 构造 URI，由框架统一编码一次；
            // 切勿再手动 URLEncoder.encode（否则 RestTemplate 二次编码导致 juhe 收到
            // 编码后的字符串而非 JSON，报 error_code=205404 模板变量不符合规范）。
            URI uri = UriComponentsBuilder.fromHttpUrl(API_URL)
                    .queryParam("mobile", mobile)
                    .queryParam("tpl_id", tplId)
                    .queryParam("vars", "{\"code\":\"" + code + "\"}")
            ...
            Map<String, Object> resp = restTemplate.exchange(
                    uri, HttpMethod.POST, null, Map.class).getBody();
