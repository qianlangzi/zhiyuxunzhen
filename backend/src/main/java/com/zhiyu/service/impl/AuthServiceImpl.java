...
public class AuthServiceImpl implements AuthService {
    ...
    public void resetPassword(ResetPasswordRequest req) {
        String phone = req.getPhone().trim();
        // 校验一次性短信验证码并消费（错误次数超限会删除验证码重新获取）
        smsCodeService.verifyAndConsume(phone, req.getCode());
        ...
        SysUser user = userMapper.selectOne(
                new com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper<SysUser>()
                        .eq(SysUser::getPhone, phone));
        if (user == null) {
            log.warn("忘记密码重置失败，手机号未绑定账号: {}", maskPhone(phone));
            // 与短信登录一致：不区分账号是否存在，避免被枚举探测手机号
            throw new BizException(ResultCode.PHONE_OR_CODE_ERROR, "该手机号未绑定账号");
        }
        if (user.getStatus() != null && user.getStatus() == 1) {
            throw new BizException(ResultCode.ACCOUNT_FROZEN);
        }
        // 校验新密码不与当前密码相同（可选，提升体验）
        if (passwordEncoder.matches(req.getNewPassword(), user.getPasswordHash())) {
            throw new BizException(ResultCode.PASSWORD_SAME_AS_OLD);
        }

        // 重置密码：更新哈希 + 递增凭证版本（作废所有已签发 token）+ 清除强制改密标志
        int newVersion = (user.getCredentialVersion() == null ? 0 : user.getCredentialVersion()) + 1;
        userMapper.update(null,
                new com.baomidou.mybatisplus.core.conditions.update.UpdateWrapper<SysUser>()
                        .eq("id", user.getId())
                        .set("password_hash", passwordEncoder.encode(req.getNewPassword()))
        ...
        auditLogService.record("reset_password", "user", user.getId(), null,
                "{\"mustChangePassword\":false,\"credentialVersion\":" + newVersion + "}");
        log.info("忘记密码重置成功: userId={} newVersion={}", user.getId(), newVersion);
    }
