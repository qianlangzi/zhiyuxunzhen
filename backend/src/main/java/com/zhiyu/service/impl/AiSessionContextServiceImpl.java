...
public class AiSessionContextServiceImpl implements AiSessionContextService {
    ...
    public AiSessionContextVO getContext(Long sessionId, Long studentId) {
        // 评估归档场景：会话已被 finish() 置为 status=1 后才回调 AI 评估，
        // 必须允许读取"已结束"会话的上下文，否则 evaluate_and_archive 永远拿不到对话
        // → osce_score_json 永久为 NULL（2026-09-02 实测修复）。仅校验归属，不校验状态。
        ChatSession session = requireOwnedSession(sessionId, studentId, true);
    ...
    @Override
    public AiSessionContextVO getCaseContext(Long caseId) {
        // SP 开场白专用：不依赖会话存在（会话刚创建时外层事务可能未提交，按 sessionId
        // 回查会 1404），直接按 caseId 取病例配置。归属/公开校验由 start() 已前置完成。
    ...
    private ChatSession requireOwnedSession(Long sessionId, Long studentId) {
        return requireOwnedSession(sessionId, studentId, false);
    }

    private ChatSession requireOwnedSession(Long sessionId, Long studentId, boolean allowEnded) {
        ...
        if (!allowEnded && (session.getStatus() == null || session.getStatus() != 0)) {
