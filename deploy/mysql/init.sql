...
CREATE TABLE IF NOT EXISTS assignment (
    ...
    start_time               DATETIME     NULL COMMENT '开始时间（定时发布），NULL=立即发布',
    late_deadline            DATETIME     NULL COMMENT '补交截止时间（allow_late_submit=1 时生效）',
    total_score              DECIMAL(6,2) NULL COMMENT '作业总分',
    score_publish_mode       VARCHAR(20)  NOT NULL DEFAULT 'IMMEDIATE' COMMENT '成绩公布 IMMEDIATE/AFTER_DEADLINE/MANUAL',
    answer_publish_mode      VARCHAR(20)  NOT NULL DEFAULT 'AFTER_DEADLINE' COMMENT '答案公布 IMMEDIATE/AFTER_DEADLINE/MANUAL',
    shuffle_questions        TINYINT(1)   NOT NULL DEFAULT 0 COMMENT '题目乱序',
    max_attempts             INT          NOT NULL DEFAULT 1 COMMENT '允许提交次数',
    plagiarism_check         TINYINT(1)   NOT NULL DEFAULT 0 COMMENT '抄袭检测',
