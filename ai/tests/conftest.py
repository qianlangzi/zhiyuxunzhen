...
def mock_settings(monkeypatch):
    ...
    LLM 配置在此被显式置空为「未配置」基线，使测试与开发者本机 .env
    完全解耦：不管 .env 里是否填了真实 LLM 地址/密钥，``settings.llm_configured``
    在本套件中默认恒为 False。需要验证「已配置」分支的用例再自行覆盖。
    """
    monkeypatch.setattr(settings, "env", "test")
    # 影响 llm_configured 的字段只有 llm_base_url 与 llm_api_key，全部显式覆盖
...
def mock_llm_client(monkeypatch):
    ...
    async def fake_report_model_event(
        event_type, model_name, error_msg, *, trace_id="-",
        capability="", recovered=False, detail=None,
    ):
        pass
