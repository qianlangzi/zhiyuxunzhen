...
async def build_bm25_index(
    ...
    try:
        ...
        def _raw_rows():
            # 优先用 query_iterator 滚动分页：offset 窗口被 Milvus 限制在 16384 内，
            # 集合超过该规模（如知识库扩充后）时 query 分页会报
            # "invalid max query result window"，导致 BM25 整链降级 dense。
            try:
                rows = []
                ...
                # 老版本 pymilvus 无 query_iterator：退回 offset 分页（<=16384 时可取全）
                rows = []
                offset = 0
                while True:
        ...
        data = await asyncio.to_thread(_raw_rows)
