...
def _split_by_sentences(text: str, max_len: int = 800) -> list[str]:
    ...
    单个超长句（无标点可切）会按 max_len 硬切，避免一条巨型文本块
    （如 MMORE 整本输出）导致 embedding 超限失败。
    """
    if len(text) <= max_len:
...
def label_mmore_chunks(
    ...
    try:
        ...
        # 4. 逐 chunk 打标。
        #    MMORE 常把整本教材输出为单个大文本块（无页码），
        #    需按句切分避免单块超长导致 embedding 失败；
        #    图片（base64）无页码，按阅读顺序分发到切分后的分块。
        clean_chunks: list[CleanChunk] = []
        for chunk_data in mmore_chunks:
            ...
            # 图片处理：base64 → 临时文件（按顺序，供分块关联）
            img_files: list[str] = []
            images = chunk_data.get("images", []) or []
            for i, img_b64 in enumerate(images):
                try:
                    img_data = _b64.b64decode(img_b64)
                except Exception:
                    continue
                img_path = os.path.join(img_dir, f"p{page}_img{i}.png")
                with open(img_path, "wb") as f:
                    f.write(img_data)
                img_files.append(img_path)

            # 按句切分（与 PyMuPDF 降级链路一致：max_len=800）
            pieces = _split_by_sentences(text, max_len=800)
            for piece_idx, piece in enumerate(pieces):
                if not piece or is_garbage(piece):
                    continue
                # 每个分块至多关联一张图（超出图数的分块不带图）
                img_path = img_files[piece_idx] if piece_idx < len(img_files) else None
                clean_chunks.append(CleanChunk(
                    text=piece,
                    page=page,
                    subject=subj,
