...
async def evaluate(case_context: str, history: list[dict[str, str]], trace_id: str = "-") -> dict[str, Any]:
    ...
    result.setdefault("patientScore", sum(scores.values()))
    result.setdefault("patientComment", "")
    try:
        result["patientScore"] = max(0.0, min(100.0, float(result["patientScore"])))
    except (TypeError, ValueError):
        result["patientScore"] = sum(scores.values())
