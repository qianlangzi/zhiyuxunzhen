...
async def process_pdf(file: UploadFile = File(...)):
    ...
    # filename 可能带子目录（如对象存储 key "ebooks/xxx.pdf"），
    # 需先确保父目录存在，否则 open 写入会 FileNotFoundError
    ...
    try:
        ...
        # Dispatcher 配置：使用 fast 模式（无需本地模型）+ 提取图片（供多模态向量）
        dispatcher_config = DispatcherConfig(
            output_path=output_dir,
            use_fast_processors=True,  # fast 模式不依赖本地模型
            extract_images=True,       # 提取图片，用于 Qwen3-VL 多模态向量
        )
        ...
        with open(merged_file) as f:
            for line in f:
                if not line.strip():
                    continue
                doc = json.loads(line)
                # MMORE MultimodalSample.to_dict() 输出顶层键：
                #   {"text": 整本全文, "modalities": [{"type","value"}], "metadata": {...}}
                # （注意：不是 content/path 键，之前解析用错导致 chunks 恒为空）
                text = doc.get("text", "") or ""
                # 提取图片（modalities 中 type=image 的 value 为临时文件路径）
                images = []
                modalities = doc.get("modalities", []) or []
                for mod in modalities:
                    if mod.get("type") == "image":
                        img_path = mod.get("value", "")
                        if img_path and os.path.exists(img_path):
                            with open(img_path, "rb") as img_file:
                                b64 = base64.b64encode(img_file.read()).decode("utf-8")
                                images.append(b64)

                # 页码：fast 模式 metadata 无 page 字段，默认 0（由 worker 端按章节重标）
                page = 0
