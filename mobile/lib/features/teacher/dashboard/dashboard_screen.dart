...
class DashboardScreen extends ConsumerStatefulWidget {
  const DashboardScreen({super.key, this.classId});

  /// 从班级详情进入时预选该班；为空则默认全部
  final int? classId;
...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  Future<void> _loadDashboard() async {
    ...
    try {
      data = await TeacherService().getDashboardOverview(classId: _selectedClassId);
    } catch (e) {
  ...
  Future<void> _loadInsight() async {
    ...
    final result = await TeacherService().getClassInsight(classId: _selectedClassId);
    if (!mounted) return;
          ...
          children: [
            AppBackAppBar(
              ...
              action: Row(
                ...
                children: [
                  AppGhostButton(
                    label: _aiInsightLoading ? '洞察中…' : 'AI 洞察',
                    small: true,
                  ...
                  AppGhostButton(
                    label: '诊断报告',
                    small: true,
                    onPressed: () => context.pushNamed(RouteNames.diagnosisReports),
                  ),
                ],
              ),
            ),
  ...
  Widget _buildClassSelector() {
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          MonoText('按班级查看学情', fontSize: 11, color: AppColors.text3Of(context)),
          const SizedBox(height: 8),
          SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: Row(
              children: [
                _classChip(label: '全部', selected: _selectedClassId == null, onTap: () => _onSelectClass(null)),
                ..._classes.map((c) {
                  final id = (c['id'] as num?)?.toInt();
                  return _classChip(
                    label: c['name'] as String? ?? '班级',
                    selected: _selectedClassId == id,
                    onTap: () => _onSelectClass(id),
                  );
                }),
              ],
            ),
  ...
  Widget _classChip({required String label, required bool selected, required VoidCallback onTap}) {
    return Padding(
  ...
  void _onSelectClass(int? classId) {
  ...
  Widget _buildStatGrid() {
    ...
    final stats = <(String, String, String, Color)>[
      ...
      ('过度检查率', '${(overExamRate * 100).toStringAsFixed(0)}%', '超均费用占比', AppColors.vermilionOf(context)),
    ];
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              MonoText(trend, fontSize: 11, color: isNegativeTrend ? AppColors.vermilionOf(context) : AppColors.primaryOf(context)),
            ],
          ),
        );
      },
  ...
  Widget _buildOsceSection(BuildContext context) {
    ...
    final osceRows = osceDimScores.entries.map((e) {
      ...
      final color = score >= 85 ? AppColors.moss : (score >= 75 ? AppColors.amberOf(context) : AppColors.vermilionOf(context));
      ...
      child: Row(
        children: [
          ...
          SizedBox(
            width: 60,
            height: 4,
            child: AppProgressBar(value: barPercent, height: 4, foregroundColor: AppColors.vermilionOf(context), radius: 2),
          ),
          const SizedBox(width: 10),
          SizedBox(
            width: 36,
