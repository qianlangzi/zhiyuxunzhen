...
/// 强制改密页
///
/// 触发场景：后端 `LoginResponse.mustChangePassword = true`（导入学生首次登录）。
/// 路由守卫在 `mustChangePassword` 为 true 时强制重定向到本页，
/// 用户完成改密后清除标记并按角色进入学生/教师首页。
///
/// 设计要点：
/// 1. 不提供返回按钮，避免用户绕过强制改密流程；
/// 2. 新密码复杂度与后端保持一致（8-32 位、字母+数字混合）；
/// 3. 改密成功后使用 `go()` 而非 `push()` 跳转，清空路由栈。
...
class _ChangePasswordScreenState extends ConsumerState<ChangePasswordScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Issue1 P0：PopScope 禁止系统返回手势/返回键离开改密页，
    // 防止用户绕过强制改密流程；退出登录按钮作为唯一逃生出口。
    return PopScope(
      canPop: false,
      child: Scaffold(
