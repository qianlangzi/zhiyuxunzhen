...
class ResetPasswordOk extends ResetPasswordResult {
  const ResetPasswordOk();
}

class ResetPasswordFail extends ResetPasswordResult {
...
class AuthApi {
  ...
  /// 忘记密码重置
  ///
  /// 对接后端 `PUT /api/v1/auth/reset-password`，请求体
  /// `{phone, code, newPassword}`（无需登录、无需旧密码）。
  /// - 校验并消费一次性短信验证码（错误次数超限需重新获取验证码）→ code=2007
  /// - 手机号未绑定账号 → code=2007
  /// - 账号已冻结 → code=2002
  /// - 新密码与当前密码相同 → code=2014
  /// - 密码强度：8-32 位、字母+数字混合
  /// 成功返回 code=0，调用方引导用户回登录页用新密码登录。
  Future<ResetPasswordResult> resetPassword({
    ...
    try {
      ...
      final data = resp.data;
      if (data == null) {
        return const ResetPasswordFail('重置失败：服务端未返回数据');
      }
      final codeVal = data['code'];
      // 严格判断：仅 code === 0 视为成功
      if (codeVal is! int || codeVal != 0) {
        final msg = data['message']?.toString() ?? data['msg']?.toString() ?? '重置失败';
        if (codeVal == 2014) {
          return const ResetPasswordFail('新密码不能与旧密码相同');
        }
        return ResetPasswordFail(msg);
      }
      return const ResetPasswordOk();
    } on DioException catch (e) {
      return ResetPasswordFail(_mapDioError(e, '重置密码'));
    } catch (e) {
      log('resetPassword 异常: $e', name: 'auth_api');
      return ResetPasswordFail('重置失败：$e');
    }
  }
