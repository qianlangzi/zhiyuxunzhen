...
class AuthService {
  ...
  /// 忘记密码重置
  ///
  /// Mock 模式：仅做本地密码格式校验，通过即视为成功。
  /// 真实模式：调用后端 `PUT /api/v1/auth/reset-password`（手机号短信验权重置）。
  Future<({String? error})> resetPassword({
    required String phone,
    required String code,
    required String newPassword,
  }) async {
    ...
    return switch (result) {
      ResetPasswordOk() => (error: null),
      ResetPasswordFail(:final message) => (error: message),
    };
  }
