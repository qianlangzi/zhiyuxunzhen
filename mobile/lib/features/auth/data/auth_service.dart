...
class AuthService {
  ...
  Future<({UserModel? user, String? error, String? token})> register({
  ...
  Future<({UserModel? user, String? error, String? token})> _mockRegister(
    String phone,
    String code,
    String username,
    String password,
    ...
    if (!_mockVerifyCode(phone, code)) {
      return (user: null, error: '验证码错误、已过期或未获取', token: null);
    }
    final store = RegisteredUserStore();
    if (await store.existsByUsername(username.trim())) {
      return (user: null, error: '该用户名已被注册，请更换', token: null);
    }
    ...
    return (user: user, error: null, token: null);
  }

  Future<({UserModel? user, String? error, String? token})> _realRegister(
    String phone,
    String code,
    String username,
    String password,
    ...
    return switch (result) {
      RegisterOk(:final user, :final token) => (user: user, error: null, token: token),
      RegisterFail(:final message) => (user: null, error: message, token: null),
    };
  }
